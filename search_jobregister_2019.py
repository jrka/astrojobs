...
# Major changes from previous script:
# - Manually enter the years/months to find job ads. The AAS job register archive
#   only lists the past 12 months, but previous months are in fact available
#   using the same URL scheme. 
# - Update field names, etc. to look for within the HTML to reflect new job register
#   website formatting.
# - Also grab the "country" field from job ad contact information; didn't do that before.
# - Save everything to new .csv and .pkl files with "2019" in file name; refers to 
#   this new search done in 2019, but job ad data is from June 2016 through May 2019. 
...
if docrawl:
    ...
    for year in yearlist: 
        ...
        for month in monthlist:
            print year, month       
            r=urllib.urlopen('https://jobregister.aas.org/jobs/archive/'+year+'/'+month).read()
    ...
    # Print to a file
    with open('jobregister_urls_2019.txt','w') as f:
        for item in newlist:
            f.write("%s\n" % item)
else:
    with open('jobregister_urls_2019.txt') as f:
...
if doscrape:
    ...
    for u, url in enumerate(newlist[startind:]): # For each listing in the job register...
        r=urllib.urlopen('http://jobregister.aas.org/'+url[1:]).read() # open contents
        ...
        if u>0 and np.mod(u+startind+1,100)==0 or (u+startind==len(newlist)-1):
            ...
            pfile='alljobs2019_'+str(data[ind1]['i']).zfill(5)+'to'+str(u+startind).zfill(5)+'.pkl'
            print pfile
...
pickles=glob.glob('./alljobs2019_*to*.pkl')
data=[]
...
df.to_csv('./jobregister_table_2019.csv',
